...
test('mobile Publisher defaults to the three first-pass platforms and still requires final approval', () => {
  ...
  assert.match(createSource, /brought the revised postcard back into view/)
  assert.match(createSource, /requestedAction = \/short/)
  assert.match(createSource, /materially different from the current caption/)
  assert.match(createSource, /returned the same caption/)
  assert.match(createSource, /scrollIntoView\(\{ behavior: 'smooth', block: 'start' \}\)/)
  assert.match(createSource, /Caption updated from your chat request/)
