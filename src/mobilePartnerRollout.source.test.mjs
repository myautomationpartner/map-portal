...
test('mobile Scheduled separates queued posts from drafts that still need review', () => {
  ...
  assert.match(appCssSource, /\.mobile-scheduled-overflow-trigger[\s\S]{0,500}width: 2\.75rem/)
})
...
test('mobile Partner chat stays inline, supports multiline drafts, and opts into iPhone safe areas', () => {
  ...
  assert.match(appCssSource, /\.mobile-voice-composer-send \{[\s\S]{0,500}touch-action: manipulation/)
...
test('installed phone portals refresh onto the current release instead of keeping stale attachment behavior', () => {
  ...
  assert.match(serviceWorkerSource, /map-portal-shell-v5-/)
  assert.match(serviceWorkerSource, /new URL\('\.\/', registrationScope\)\.href/)
  assert.match(serviceWorkerSource, /NAVIGATION_NETWORK_BUDGET_MS = 1_500/)
  assert.match(serviceWorkerSource, /request\.mode === 'navigate'/)
  assert.match(serviceWorkerSource, /cachedPortalShell\(cache, request\)/)
  assert.doesNotMatch(serviceWorkerSource, /client\.navigate\(clientUrl\.href\)/)
...
test('mobile Publisher defaults to the three first-pass platforms and still requires final approval', () => {
  ...
  assert.match(createSource, /const previewUrls = mediaUrls\.length \? mediaUrls : \(imagePreview \? \[imagePreview\] : \[\]\)/)
  assert.match(createSource, /previewUrls,/)
  assert.match(createSource, /data-has-media=\{hasMedia \? 'true' : 'false'\}/)
...
test('phone portal uses warm cream/ink tokens and full Final review captions', () => {
  assert.match(appCssSource, /--pulse-bg: #F6F3EE;/)
  assert.match(appCssSource, /--pulse-ink: #12262B;/)
  assert.match(appCssSource, /--pulse-text: #1A1814;/)
  assert.match(appCssSource, /--pulse-muted: #5E554D;/)
  assert.match(appCssSource, /--pulse-aqua: #2F7A7E;/)
  assert.doesNotMatch(appCssSource, /--pulse-bg: #f4f7f8;/)
  assert.doesNotMatch(appCssSource, /--pulse-aqua: #00aeb6;/)
  assert.match(appCssSource, /\.mobile-partner-generated-caption p \{[\s\S]{0,280}white-space: pre-wrap/)
  assert.doesNotMatch(appCssSource, /\.mobile-partner-generated-caption p \{[\s\S]{0,500}-webkit-line-clamp: 4/)
  assert.doesNotMatch(appCssSource, /\.mobile-partner-post-caption p \{[\s\S]{0,500}-webkit-line-clamp: 3/)
  assert.match(appCssSource, /\.mobile-partner-generated-platforms \{[\s\S]{0,500}flex-wrap: wrap/)
  assert.match(appCssSource, /\.mobile-partner-generated-actions \.mobile-partner-primary \{[\s\S]{0,500}background: var\(--pulse-ink\)/)
  ...
  assert.match(navSource, /label: 'Post'/)
  assert.match(navSource, /label: 'Files'/)
  assert.match(navSource, /fixed inset-x-0 bottom-0/)
  assert.match(navSource, /#C9A84C/)
...
test('empty mobile Post lets you type a post or ask Partner', () => {
  ...
  assert.match(createSource, /Write it/)
  assert.match(createSource, /Ask Partner/)
...
test('Ask Partner talks first on what-to-post using Radar, then drafts on pick', () => {
  ...
  assert.match(mobileChatSource, /pickFreshOpportunities/)
  assert.match(mobileChatSource, /Looking at this week/)
  assert.match(mobilePartnerIntentSource, /pickFreshOpportunities/)
  assert.match(mobilePartnerIntentSource, /formatTopicOptionsMessage/)
  assert.match(mobilePartnerIntentSource, /I looked at this week/)
  assert.doesNotMatch(mobilePartnerIntentSource, /Here is what I would post about right now/)
