...
Legacy/dedicated tenant workers can keep `workers_dev = true` so the technical host remains available as a fallback while the MAP-owned customer-facing URL stays official. Shared-path tenants use `myautomationpartner.com/portal/<client-slug>`.

If a dedicated worker is ever temporarily reachable on a technical host such as `*.workers.dev`, set:

- `PORTAL_CANONICAL_HOST=<approved-customer-host>`

Current shared-worker behavior:

- non-API `GET` / `HEAD` requests on technical hosts redirect to the approved MAP-owned customer URL
